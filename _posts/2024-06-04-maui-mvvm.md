  ...
  * **ViewModel**
    ...
    * Responsabilidades
      ...
      * Es el componente que maneja la lógica de navegación.
      ...
      > Para menajar las interacciones del usuario con la UI como por ejemplo hacer tap en un botón o similar podremos usar los **comandos implementando la interface ICommand**.
...
* Si vamos en crudo, simplemente tenemos que implementar la inferface **INotifyPropertyChanged**. Este será el ejemplo que utilizaremos para este artículo.
* Si no vamos en crudo y nos apoyamos en algún framework o "kit de herramientas" tenemos que implementar en el caso de [CommunityToolkit.Mvvm](https://learn.microsoft.com/es-es/dotnet/communitytoolkit/mvvm/) la clase abstracta **ObservableObject**. Este ejemplo lo veremos en el siguiente artículo.
...
{
    // Método que tendrán que implementar todos los ViewModels que hereden esta clase y puede ser utilizado para recuperar por ejemplo datos de una API
...
* **LaunchesViewModel**    
  * Tendrá una **propiedad** llamada **LaunchesResponse** que será utilizada por la vista y como hemos comentado **implementará la interfaz INotifyPropertyChanged para notificar que su valor ha cambiado**, por lo que conseguimos que la UI se actualice de forma automática.
...
> El código de los ViewModels que he incluído no está completo, he omitido los constructores y algunos fields tales como servicios.

Aquí dejo el acceso al repositorio en GitHub por si quieres echarle un vistazo y tener todo el código de ejemplo completamente funcional.
